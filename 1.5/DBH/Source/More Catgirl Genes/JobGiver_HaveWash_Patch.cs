...
namespace More_Catgirl_Genes
{
    ...
    class JobGiver_HaveWash_Patch
    {
        ...
        {
            if (!pawn.genes.HasActiveGene(InternalDefOf.BBLK_Tongue_Cleaning)) return true;
            BodyPartRecord tongue = pawn?.health?.hediffSet?.GetBodyPartRecord(InternalDefOf.Tongue);
            if (tongue == null || (!pawn.health.hediffSet.PartOrAnyAncestorHasDirectlyAddedParts(tongue) && pawn.health.hediffSet.PartIsMissing(tongue))) return true;
